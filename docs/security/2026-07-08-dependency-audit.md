...
- `@aws-sdk/*` and `fast-xml-parser` appear through `@types/nodemailer` in the installed tree.
- `ws` appears through Supabase Realtime.
- `brace-expansion`, `minimatch`, and `form-data` remain as transitive items in the installed dependency graph.
