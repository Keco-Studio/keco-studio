...
- Paid acceptance BLOCKED by the provider: authenticated owner `seed-project` created authoritative V3 map `852ea11e-a80d-47a3-b999-62824415c26f` and revision `e9a25ff5-39fd-466d-beb7-ffadb596200e` through public RPCs, then called the current standalone Edge function with the user's access token and explicit paid confirmation. PixelLab returned `pixellab_rate_limited` during capability discovery, before provider submission. The single asset remains `blocked` with no provider job, no stored image, and attempt count `0`, so no duplicate charge was created; live dimensions, hash prefix, private read-back, and restored-browser screenshot remain unverified.
