...
emails used for signup using a six-digit code delivered to the real mailbox,
and keep all account and permission ownership tied to the Supabase Auth user
UUID. Preserve the existing password-recovery email-link flow. User-initiated
email changes are deferred.

Deliver in two stages. Stage 1 implements current-email uniqueness,
normalization, profile synchronization, safe UUID-bound invitations, a
read-only Account email view, and repairs to the existing password-recovery
pages. It does not introduce a six-digit code UI, enable mandatory signup
confirmation, or claim that signup email ownership has been verified. Stage 2
enables confirmed signup, adds six-digit signup code entry and resend, and
validates delivery to real mailboxes through the hosted Auth SMTP and
templates. Password recovery keeps its one-time email link in both stages.
Neither stage exposes user-initiated email changes. Stage 1 is deployable for
identity consistency but does not provide the anti-impersonation guarantee of
Stage 2.
...
- An email can be bound to at most one existing Auth account at a time,
  including accounts created through OAuth. Concurrent registration must not
  bypass this rule.
- In Stage 1, signup continues using the existing Auth email confirmation
  ...
  second user record.
- The Account page displays the current email as read-only. It provides no new
  email field, change action, or call to the Auth email-update API.
...
trigger synchronizes it when Auth's effective current email changes through an
administrative or provider operation. This preserves one authoritative source
without exposing a user-facing change flow.
...
account area shows the authoritative current email as read-only. Do not expose
Auth service-role credentials, call the Auth email-update API from the Account
page, or permit clients to write the profile email as if it were authoritative.

## Email Verification and Delivery

In Stage 2, use Supabase Auth's signup OTP generation/verification rather than
storing application-generated codes. The interface sends/resends a code,
...
production Auth mail reaches real inboxes. Password-recovery links still
depend on working Auth mail delivery. Production delivery, suppression, rate
limits, and OTP templates are a Stage 2 acceptance gate.
...
If an Auth email changes through an administrative or provider operation, the
pending invitation stays assigned to the original user. On deletion a
recipient-UUID foreign key cascades away pending invites for that user. Legacy
...
- Stage 1: unit-test normalization, duplicate/race handling, Auth-to-profile
  sync, invite UUID validation, read-only Account state, and password rules.
- Stage 1: integration-test recovery links; end-to-end test case-insensitive
  duplicates, deleting/re-registering with a new UUID, and a pending invite
  not moving to a new owner of its former email.
- Stage 2: integration-test signup/code delivery to local Mailpit, wrong or
  expired codes, and resend cases.
...
Out of scope: user-initiated email changes, a permanent email history, custom
OTP storage, username-based password recovery, automatic merging of OAuth
identities, moving old project data to a new UUID, and changes to unrelated
Keco Admin or GDD work.
