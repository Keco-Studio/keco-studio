...
Validate the package with the bundled `validate_generated_asset_package.py` before materialization; it checks declared SHA-256, PNG dimensions, `tileSize * columns/rows`, layout identity, and `terrainMapping`. Static evidence must prove dimensions, layout metadata, resource paths, and terrain configuration. Runtime evidence must include the loaded snapshot hash and a machine-readable `KECO_OBSERVATION`. Missing mapping, mismatched tile geometry, or an atlas-only export used as terrain-ready is a failure or `blocked_before_write`, not a warning.
