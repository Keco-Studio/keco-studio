...
1. Read all seven files in `references/` completely before calling Keco, PixelLab, or Godot tools: [source-priority.md](references/source-priority.md), [slice-plan.md](references/slice-plan.md), [data-plan.md](references/data-plan.md), [asset-plan.md](references/asset-plan.md), [eval-spec.md](references/eval-spec.md), [godot-mcp-policy.md](references/godot-mcp-policy.md), and [recovery-policy.md](references/recovery-policy.md). When the slice needs an image, also read the shared [PixelLab capability registry](../../references/pixellab-capability-registry.md).
...
7. If the slice needs UI images, create the AssetPlan and resolve a live PixelLab operation profile against the shared [PixelLab capability registry](../../references/pixellab-capability-registry.md) before any Keco, asset, or Godot write. Select the provider capability from `assetKind` first, then check the live MCP adapter and record `compatibility: exact|fallback|unavailable`. An `unavailable` capability is `blocked_before_write`; a `fallback` needs its accepted difference recorded, or user confirmation when the difference affects acceptance. Existing UI extensions require the original UI image as a generation reference; new UI must derive references and constraints from the current project's UI. Do not substitute an unrelated image generator to work around an unavailable capability.
8. In `WRITE_KECO_DATA`, discover the project's compatible asset registry from fresh table schemas before planning rows. Reuse a same-purpose table by semantic fields regardless of its display name ( `UI Assets`, `Generated Assets`, `Assets`, or another project-specific name); create a new table only when no compatible target exists and record that discovery evidence. After every successful Keco write, paginate-read all affected tables again and refresh IDs, revisions, timestamps, labels, values, and references before continuing.
9. Generate only planned AssetPlan outputs with PixelLab, using the operation recorded in the accepted operation profile. Validate each local result before upload; never expose or persist the PixelLab API key.
...
11. Export only the fresh Keco read-back with `${CLAUDE_PLUGIN_ROOT}/scripts/export_keco_snapshot.py`, validate it with `${CLAUDE_PLUGIN_ROOT}/scripts/validate_snapshot.py`, then materialize the Godot image from the Keco image URL and require its SHA-256 to match the Keco row. The running game must expose the loaded snapshot hash.
...
| Call a remembered PixelLab tool name | Resolve the live adapter from the shared capability registry |
