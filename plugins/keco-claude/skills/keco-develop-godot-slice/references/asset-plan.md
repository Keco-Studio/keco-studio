...
| Provider Capability | string | Capability key from the shared registry |
| Transport Tool | string | Live MCP tool actually called |
| Compatibility | enum | `exact`, `fallback`, or `unavailable` |
...
Resolve the UI generation operation against the shared [PixelLab capability registry](../../../references/pixellab-capability-registry.md). Select the provider capability from `assetKind: ui` first (`ui-elements-pro`, then `style-pro`), then list the live PixelLab MCP tools and record `providerCapability`, `officialEndpoint`, `transportTool`, and `compatibility: exact|fallback|unavailable`. Inspect the live input schema before calling it and map the AssetPlan prompt, dimensions, transparency, and reference images to the declared fields. Do not guess unsupported parameters, do not call an operation by memory, and do not silently substitute an unrelated image generator. An `unavailable` capability blocks the asset before any write.
...
transportTool: live MCP tool name
