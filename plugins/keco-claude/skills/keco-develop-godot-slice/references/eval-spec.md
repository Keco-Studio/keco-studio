...
For PixelLab outputs, file type, dimensions, alpha, non-empty pixels, reference hashes, and output hash are automated evidence. Style consistency with the existing UI, readability in context, and integrated appearance remain `manualRequired: true`. A successful generation response never passes a visual evaluation by itself.
