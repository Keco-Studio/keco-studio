...
| PixelLab unavailable, or the resolved capability is `unavailable` for a required asset | Stop before Keco, asset, or Godot writes and report the AssetPlan blocker with the capability key |
| Resolved capability is `fallback` and the difference affects acceptance | Ask the user before generating; record the accepted difference otherwise |
