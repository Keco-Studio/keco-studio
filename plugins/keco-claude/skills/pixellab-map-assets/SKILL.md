...
- [ ] resolve each need to a capability key in the shared registry
...
Read the shared [PixelLab capability registry](../../references/pixellab-capability-registry.md). It is the single source of truth for capability keys, official endpoints, live MCP adapters, and the canonical `assetKind` vocabulary used by every skill in this plugin.

List the currently exposed PixelLab MCP tools and inspect the exact schema before making a call. Match by semantic capability, not by memory. Resolve each map need to a capability key:

| Asset need | `assetKind` | Capability key | Output expectation |
|---|---|---|---|
| seamless top-down terrain | `tileset` | `top-down-tileset` | PNG tile set plus Wang/corner autotiling metadata |
| roads, rivers, or paths | `tile` | `tiles-pro` | directional edge/path tiles and edge rules |
| floors, walls, doors, columns | `tile` | `tiles-pro` | compatible building tiles/parts |
| one world prop or building | `image` | `pixflux` or `bitforge` | transparent-background PNG object |

**Several of these capabilities have no live MCP adapter today.** The registry records the current relation for each. Record `providerCapability`, `officialEndpoint`, `transportTool`, and `compatibility: exact|fallback|unavailable` in the asset manifest. An `unavailable` capability blocks that asset before any write; report the missing capability and ask whether the user accepts a different operation. Do not silently substitute a generic image generator, and never call a remembered tool name such as `create_topdown_tileset` or `create_map_object` -- those are planning labels, not live MCP tools. Never send secrets in prompts or tool arguments.
...
assetKind: tileset
providerCapability: top-down-tileset
transportTool: live MCP tool name or null
...
For a tile or tileset package bound for a Godot slice, run `${CLAUDE_PLUGIN_ROOT}/scripts/validate_generated_asset_package.py <package.json>` to check declared hashes, real PNG dimensions, layout identity, and terrain mapping before integration.
...
- each capability key resolved, the live transport tool actually called, its recorded `compatibility`, and the generated files;
...
- unresolved blockers, including unavailable capabilities or missing MCP configuration.
