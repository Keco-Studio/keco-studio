...
`test8-24/game-gdd` is the authoritative design source for a Slice. Before
decomposition, create a versioned Requirement Inventory with a stable
...
GDD plans and EvalReports must set `coverageMode: gdd`, include `gddSource`
(`revision`, `contentHash`, and `inventoryHash`), and be validated with the
same inventory file. Omitting coverage fields is only allowed for explicitly
legacy or non-GDD runs, never for this workflow.
...
approval, merge it into `test8-24/game-gdd` or add an explicit accepted-patch
reference in that document, then re-read the document and refresh its revision,
