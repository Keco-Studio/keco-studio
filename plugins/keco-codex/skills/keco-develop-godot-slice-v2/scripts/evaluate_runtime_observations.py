...
def main() -> int:
    ...
    try:
        ...
        if not isinstance(raw_spec, dict) or set(raw_spec) != {"evaluations"}:
            raise ValueError("EvalSpec must contain only evaluations")
        ...
        by_id = {item.get("evalId"): item for item in specs if isinstance(item, dict)}
