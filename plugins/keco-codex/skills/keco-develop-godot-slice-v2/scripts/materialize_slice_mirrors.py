...
def validate_manifest(value: Any) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if schema_version == 2:
        ...
        optional = {"ok", "legacyLayout"}
        if set(value) - optional != required:
            fail("version 2 mirror manifest fields are invalid")
        if value.get("contractVersion") != 2 or value.get("legacyLayout", False) is not False:
        ...
        optional = {"ok", "contractVersion", "legacyLayout"}
        if set(value) - optional != common:
            fail("legacy mirror manifest fields are invalid")
        if value.get("contractVersion", 1) != 1 or value.get("legacyLayout", True) is not True:
            fail("legacy mirror manifest identity is invalid")
        if not valid_integer(value.get("currentSequence"), 0):
            fail("legacy export sequence is invalid")
        contract_version = 1
    else:
        fail("mirror manifest schema version is invalid")
    if (
    ...
    if contract_version == 2 and len(files) != 3:
    ...
    paths: set[str] = set()
    kinds: set[str] = set()
    legacy_fields = {"kind", "repositoryPath", "documentId", "epoch", "revision", "byteCount", "sha256", "content"}
    v2_fields = legacy_fields | {"folderId"}
    for item in files:
        fields = v2_fields if contract_version == 2 else legacy_fields
        ...
        if (
            ...
            or (contract_version == 2 and (not isinstance(item.get("folderId"), str) or not item["folderId"]))
        ...
        if contract_version == 2 and (kind not in V2_KINDS or not canonical_v2_path(kind, relative)):
        ...
        paths.add(relative)
        kinds.add(kind)
    if contract_version == 2 and kinds != V2_KINDS:
