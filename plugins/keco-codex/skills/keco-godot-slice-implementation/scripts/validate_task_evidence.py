...
def validate_plan(value: Any) -> tuple[str, dict[str, dict[str, Any]], set[str]]:
    if not isinstance(value, dict) or value.get("schemaVersion") != 2 or not valid_hash(value.get("planRevision")):
        fail("SlicePlan must provide schemaVersion 2 and planRevision")
    ...
    for task in tasks:
        ...
        for phase in ("red", "green"):
            ...
            if not isinstance(command, dict) or not isinstance(command.get("command"), str) or not command["command"].strip() or command.get("expected") not in {"fails", "passes"}:
...
def validate_result(payload: dict[str, Any], *, run_id: str, slice_id: str, plan_revision: str, tasks: dict[str, dict[str, Any]], allowed: set[str]) -> None:
    ...
    if set(payload) != required or payload.get("schemaVersion") != 2:
