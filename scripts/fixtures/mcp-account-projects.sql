...
-- pending, and inaccessible collaborator-side projects.
...
select
  ...
  case
    when project_number <= 50 then 'aaaaaaaa-bbbb-cccc-dddd-000000000001'::uuid
...
select
  ...
  '2029-12-01 00:00:00+00'::timestamptz
from generate_series(51, 100) as project_number
...
begin
  ...
  if (v_owned, v_accepted, v_pending) <> (50, 50, 1) then
    raise exception 'MCP account fixture counts are invalid: %, %, %',
      v_owned, v_accepted, v_pending;
  end if;
end;
$$;
