...
**Created**: 2026-07-09
**Status**: Draft
**Input**: GitHub issue #176 — `findColon` only inspects the first `:`/`：`; a line like `12:30: 说话内容` has its first colon inside a time token, so the speaker separator is missed and the whole line is mis-classified as narration.
...
2. Failure case `12:30: 说话内容` (half-width, no full-width colon present): `cPos === -1`, `ePos` points at the colon in `12:30`. `line[ePos-1]`=`2` and `line[ePos+1]`=`3` are both digits, so the guard returns `-1` — reporting "no colon at all." The real speaker/content separator (the second `:`, after `30`) is never found, so the caller at `classifier.ts:355` (`const colonPos = findColon(stripped)`) treats the entire line as narration and the speaker+dialogue split is lost.
3. The same defect affects the full-width branch: `12:30：说话` returns `cPos` (correct here by luck), but a line whose only real separator sits after a leading `HH:MM:` half-width time is not scanned past the first digit-colon.
...
As a script author, a dialogue line that begins with a timestamp (e.g. `12:30: 台词`) is parsed as speaker/time + dialogue, not silently demoted to narration.
...
**Independent Test**: Unit-test `findColon` and the classifier on `12:30: 说话内容` and assert the returned position is the separator colon after `30`, and the line classifies as a dialogue/speaker node.

**Acceptance Scenarios**:
1. **Given** `12:30: 说话内容`, **When** `findColon` runs, **Then** it returns the index of the second `:` (the separator), not `-1`.
...
4. **Given** a normal speaker line `张三：台词` / `张三: 台词`, **When** classified, **Then** behavior is unchanged.

### Edge Cases

- Multiple time-like tokens then a separator: `08:00 到 12:30: 台词` → the separator is the colon after `30`.
- Mixed full/half-width: line contains a half-width time and a full-width separator `12:30：台词` → returns the full-width separator position.
- Colon at end with nothing after (`名字:`) → still treated as a separator (empty content), matching current non-time behavior.
...
- **SC-001**: A regression test on `12:30: 说话内容` fails against the current implementation (returns `-1`) and passes after the fix (returns the separator index; classifies as dialogue).
