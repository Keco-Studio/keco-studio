...
/**
 * 计算关卡收益（统一使用对数-平方根混合曲线）
 * 与综合计算器保持一致
 * 
 * 设计原则：
 * - 银币：1-50级从500到6000，50-100级到12000，100-200级到18000
 * - 经验：与等级挂钩，保持边际效用递减
 * - 首次通关：重复过关的2-3倍，额外元宝奖励
 * 
 * @param level 关卡
 * @param playerLevel 主将等级
 * @param isFirstTime 是否首次通关（首次通关奖励更高）
 */
export function calculateLevelReward(level: Level, playerLevel: number, isFirstTime: boolean = false): {
  silver: number;
  exp: number;
  gold: number;
} {
