...
export function CharactersScreen({ onContinue }: { onContinue: () => void }) {
  ...
  return <section className={styles.flowScreen} aria-labelledby="characters-title">
    <div className={styles.flowHeading}><div><span className={styles.kicker}>Roster and teams</span><h2 id="characters-title">Configure characters</h2><p>Choose imported characters and split them across both sides.</p></div><input type="search" placeholder="Search characters" value={search} onChange={(event) => setSearch(event.target.value)} /></div>
    <div className={styles.characterPicker}>{filtered.map((character) => {
