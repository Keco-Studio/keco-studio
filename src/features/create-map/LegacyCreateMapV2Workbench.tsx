...
export type LegacyCreateMapV2WorkbenchProps = {
  initialMapId: string;
  readOnly: true;
  onBack: () => void;
};

export function LegacyCreateMapV2Workbench({ initialMapId, readOnly, onBack }: LegacyCreateMapV2WorkbenchProps) {
  ...
  const canPrepareGeneration = !readOnly && canGenerate && (generation.phase === 'idle' || generation.phase === 'failed');
  ...
  const changeTool = (nextTool: MapTool) => {
    if (readOnly && nextTool !== 'select' && nextTool !== 'hand') return;
    setTool(nextTool);
  ...
  const openSavedMapRef = useRef(openSavedMap);
  openSavedMapRef.current = openSavedMap;
  ...
  const requestGeneration = () => {
    if (readOnly || !canPrepareGeneration) return;
  ...
  useEffect(() => {
    if (!initialMapId || initialOpenRef.current === initialMapId || draft.identity?.mapId === initialMapId) return;
    initialOpenRef.current = initialMapId;
    void openSavedMapRef.current({ id: initialMapId } as SavedMapSummary).catch(() => {
      initialOpenRef.current = '';
    });
  }, [draft.identity?.mapId, initialMapId]);

  return (
    <main className={styles.workbench} data-testid="create-map-workbench" data-mode={mode} data-schema-version="2" data-read-only="true">
      {(leftOpen || rightOpen) ? (
      ...
      <aside className={`${styles.leftPanel} ${leftOpen ? styles.drawerOpen : ''}`} aria-label="Map source and structure">
        ...
        />
        <SavedMapsPanel
          maps={savedMaps.maps.filter((map) => map.schemaVersion === 2)}
      ...
      <section className={styles.canvasPanel} aria-label="Map canvas">
        <header className={styles.canvasHeader}>
          ...
          <div className={styles.headerActions}>
            <button type="button" className={styles.legacyBackButton} onClick={onBack}>
      ...
      <aside className={`${styles.rightPanel} ${rightOpen ? styles.drawerOpen : ''}`} aria-label="Map plan and inspector">
        ...
        /> : (
          <section className={styles.inspectorSection} aria-label="Legacy map status">
            <span className={styles.draftBadge}>Read only</span>
            <p className={styles.inspectorNote}>V2 maps remain available for viewing. New generation uses V3.</p>
          </section>
        )}
      </aside>
    </main>
