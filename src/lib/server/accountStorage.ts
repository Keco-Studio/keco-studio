...
export async function readOwnAccountStorage(client: SupabaseClient): Promise<AccountStorageSummary> {
  const { data, error } = await client.rpc('account_storage_summary_v6');
...
export async function readProjectStorageEntities(
  ...
  let result: { data: unknown; error: unknown };
  try {
    result = await client.rpc('account_storage_project_entities_v6', {
      p_project_id: projectId,
      p_query: query,
      p_sort: sort,
      p_limit: limit,
...
export async function readProjectStorageEntityDetail(
  ...
  let result: { data: unknown; error: unknown };
  try {
    result = await client.rpc('account_storage_entity_details_v6', {
      p_project_id: projectId,
      p_entity_kind: kind,
