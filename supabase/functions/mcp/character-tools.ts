...
const characterReference = z.object({ assetId: uuid, sha256: fingerprint, role: z.enum(["style", "source"]), required: z.boolean(), usage: z.string().trim().min(1).max(240) }).strict();
const characterPlanV2 = characterPlan.omit({ schemaVersion: true }).extend({ schemaVersion: z.literal(2), references: z.array(characterReference).max(4).refine((items) => new Set(items.map((item) => item.assetId)).size === items.length) }).strict();
...
const plan = z.union([characterPlan, characterPlanV2, animationPlan]).refine((value) => value.kind !== "character" || value.width === value.height);
...
const fields = ["projectId", "assetId", "saveVersion", "status", "plan", "generation", "attemptId", "generationId", "planFingerprint", "attemptCount", "lastErrorCode", "providerJobId", "storagePath", "sha256", "width", "height", "hasTransparency", "metadata", "imageUrl", "providerDiagnostics", "referenceCompatibility", "feeNotice", "confirmationPurpose", "confirmationExpiresAt", "confirmationToken"] as const;
