...
const context = {
  ...
  supabase: {
    rpc(name: string) {
      if (name === 'mcp_begin_operation') return Promise.resolve({
        data: [{ operation_id: '00000000-0000-4000-8000-000000000002',
          remaining: 239, reset_at: new Date(Date.now() + 60_000).toISOString() }],
        error: null,
      });
      if (name === 'mcp_complete_operation') return Promise.resolve({ data: null, error: null });
...
Deno.test('tools/list exposes the editor probe, reads, and writes', async () => {
  ...
  assertEquals(message.error, undefined);
  const names = (message.result?.tools as Array<{name:string}>).map(tool => tool.name);
  assertEquals(names, [
...
Deno.test('viewer tools/list excludes every write tool', async () => {
  const message = await rpc('tools/list', {}, { ...context, role: 'viewer' } as McpRequestContext);
  assertEquals((message.result?.tools as Array<{name:string}>).map(tool => tool.name), [
    'keco_connection_probe', 'list_project_structure', 'query_table_rows',
    'list_documents', 'read_document', 'semantic_search',
  ]);
});

Deno.test('resources and prompts are discoverable', async () => {
  const resources = await rpc('resources/list');
  assertEquals((resources.result?.resources as Array<{uri:string}>).map(x => x.uri),
  ...
  assertEquals((prompts.result?.prompts as Array<{name:string}>).map(x => x.name),
...
Deno.test('strict tool schemas reject unknown project selectors before execution', async () => {
  const message = await rpc('tools/call', { name: 'list_project_structure',
...
Deno.test('prompts/get is strict and static', async () => {
  const invalid = await rpc('prompts/get', { name: 'analyze_project',
    arguments: { projectId: '22222222-2222-4222-8222-222222222222' } });
  assertEquals(invalid.error?.code, -32602);
  const valid = await rpc('prompts/get', { name: 'build_tables_from_document',
    arguments: { documentId: '22222222-2222-4222-8222-222222222222' } });
  assertEquals(valid.error, undefined);
  assertMatch(String((valid.result?.messages as Array<{content:{text:string}}>)[0].content.text),
...
Deno.test('resources/read executes a bounded project structure read', async () => {
  const calls: string[] = [];
  const resourceContext = { ...context, supabase: {
    async rpc(name: string) {
      calls.push(name);
      if (name === 'mcp_begin_operation') return { data: [{
        operation_id: '00000000-0000-4000-8000-000000000004', remaining: 119,
        reset_at: new Date(Date.now() + 60_000).toISOString(),
      }], error: null };
      if (name === 'mcp_read_project_structure') return { data: {
        project: { id: context.projectId, name: 'Bound project' },
        folders: [], tables: [], documents: [],
      }, error: null };
      return { data: null, error: null };
    },
  } } as unknown as McpRequestContext;
  const message = await rpc('resources/read', { uri: 'keco://project/structure' }, resourceContext);
  assertEquals(message.error, undefined);
  const contents = message.result?.contents as Array<{ text: string }>;
  assertMatch(contents[0].text, /Bound project/);
  assertEquals(calls, ['mcp_begin_operation', 'mcp_read_project_structure', 'mcp_complete_operation']);
});

Deno.test('create_table makes one primary atomic RPC between telemetry calls', async () => {
  const calls: Array<{ name: string; parameters?: Record<string, unknown> }> = [];
  const writeContext = { ...context, supabase: {
    async rpc(name: string, parameters: Record<string, unknown>) {
      calls.push({ name, parameters });
      if (name === 'mcp_begin_operation') return { data: [{
        operation_id: '00000000-0000-4000-8000-000000000005', remaining: 29,
        reset_at: new Date(Date.now() + 60_000).toISOString(),
      }], error: null };
      if (name === 'mcp_create_table') return { data: [{ table_id: 'new-table' }], error: null };
      return { data: null, error: null };
    },
  } } as unknown as McpRequestContext;
  const message = await rpc('tools/call', { name: 'create_table', arguments: {
    name: 'Characters', fields: [{ label: 'Name', dataType: 'string', required: true }],
  } }, writeContext);
  assertEquals(message.error, undefined);
  assertEquals(message.result?.isError, undefined);
  assertEquals(calls.map(call => call.name), [
    'mcp_begin_operation', 'mcp_create_table', 'mcp_complete_operation',
  ]);
...
Deno.test('stale document state token is rejected before codec or privileged replacement', async () => {
  const calls: string[] = [];
  const head = { id: '22222222-2222-4222-8222-222222222222', name: 'Document',
    content: '# Stored', yjs_state: 'snapshot', collab_epoch: 3,
    collab_revision: 7, updated_at: '2026-07-22T00:00:00.000Z' };
  const from = (table: string) => {
    const builder: Record<string, unknown> = {};
    for (const method of ['select', 'eq', 'order']) {
      builder[method] = () => builder;
    }
    builder.maybeSingle = async () => ({ data: head, error: null });
    builder.range = async () => ({ data: table === 'document_yjs_updates' ? [] : null, error: null });
    return builder;
  };
  const staleContext = { ...context, supabase: {
    from,
    async rpc(name: string) {
      calls.push(name);
      if (name === 'mcp_begin_operation') return { data: [{
        operation_id: '00000000-0000-4000-8000-000000000006', remaining: 29,
        reset_at: new Date(Date.now() + 60_000).toISOString(),
      }], error: null };
      return { data: null, error: null };
    },
  } } as unknown as McpRequestContext;
  const message = await rpc('tools/call', { name: 'update_document', arguments: {
    documentId: head.id, markdown: '# Replacement',
    stateToken: { epoch: 3, revision: 6, updateIds: [] },
  } }, staleContext);
  assertEquals(message.error, undefined);
  assertEquals(message.result?.isError, true);
  assertMatch(JSON.stringify(message.result), /DOCUMENT_CONFLICT/);
  assertEquals(calls, ['mcp_begin_operation', 'mcp_complete_operation']);
});

Deno.test('rate-limited probe returns a stable tool error', async () => {
  const rateLimited = { ...context, supabase: {
    rpc(name: string) {
      if (name === 'mcp_begin_operation') return Promise.resolve({ data: [{
        operation_id: '00000000-0000-4000-8000-000000000003', remaining: -1,
        reset_at: new Date(Date.now() + 10_000).toISOString(),
      }], error: null });
      throw new Error('Completion must not run for rejected admission.');
    },
  } } as unknown as McpRequestContext;
  const message = await rpc('tools/call', { name: 'keco_connection_probe', arguments: {} }, rateLimited);
  assertEquals(message.error, undefined);
  assertEquals(message.result?.isError, true);
  assertEquals((message.result?.structuredContent as Record<string, unknown>).ok, false);
