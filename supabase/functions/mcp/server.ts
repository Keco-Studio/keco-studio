...
import { runMcpProtocolOperation, type McpOperationClass } from './telemetry.ts';
import { toolFailure } from './results.ts';
import { asPublicMcpError } from './errors.ts';

const READ_TOOLS = new Set(['list_project_structure', 'query_table_rows',
  'list_documents', 'read_document']);
const WRITE_TOOLS = new Set(['create_table', 'create_table_row', 'update_table_row',
  'create_document', 'update_document']);
const STATIC_METHODS = new Set(['initialize', 'ping', 'tools/list', 'resources/list',
  'resources/templates/list', 'prompts/list', 'prompts/get', 'notifications/initialized']);
...
async function protocolEnvelope(request: Request): Promise<{
  ...
  if (request.method !== 'POST') return { id: null, method: request.method,
    operation: 'protocol_' + request.method.toLowerCase(), operationClass: 'static', requestBytes: 0 };
  let requestBytes = 0;
  try {
    ...
    if (!body || Array.isArray(body) || body.jsonrpc !== '2.0' || typeof body.method !== 'string') {
      return { id: null, method: 'invalid', operation: 'protocol_invalid_request',
        operationClass: 'static', requestBytes };
    }
    const id = typeof body.id === 'string' || typeof body.id === 'number' || body.id === null
      ? body.id : null;
    if (body.method === 'tools/call') {
      const name = body.params?.name;
      if (name === 'keco_connection_probe') return { id, method: body.method,
        operation: name, operationClass: 'static', requestBytes };
      if (name === 'semantic_search') return { id, method: body.method,
        operation: name, operationClass: 'search', requestBytes };
      if (typeof name === 'string' && READ_TOOLS.has(name)) return { id, method: body.method,
        operation: name, operationClass: 'read', requestBytes };
      if (typeof name === 'string' && WRITE_TOOLS.has(name)) return { id, method: body.method,
        operation: name, operationClass: 'write', requestBytes };
      return { id, method: body.method, operation: 'unknown_tool',
        operationClass: 'static', requestBytes };
    }
    if (body.method === 'resources/read') return { id, method: body.method,
      operation: 'resource_read', operationClass: 'read', requestBytes };
    if (STATIC_METHODS.has(body.method)) return { id, method: body.method,
      operation: 'protocol_' + body.method.replaceAll('/', '_'),
      operationClass: 'static', requestBytes };
    return { id, method: body.method, operation: 'protocol_unknown_method',
      operationClass: 'static', requestBytes };
  } catch { return { id: null, method: 'invalid', operation: 'protocol_invalid_request',
    operationClass: 'static', requestBytes }; }
}
...
export async function handleProtocolRequest(
  ...
  try {
    ...
    if (envelope.method === 'tools/call') {
      return Response.json({ jsonrpc: '2.0', id: envelope.id, result: toolFailure(error) });
    }
