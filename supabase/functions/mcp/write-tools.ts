...
import { encodeDocumentMarkdown, normalizeDocumentState } from './document-codec.ts';
...
const fieldSchema = z.object({
  ...
  dataType: z.enum(['string', 'string_array', 'int', 'int_array', 'float',
    'float_array', 'boolean', 'enum', 'date', 'reference']),
  ...
  if (field.dataType === 'enum' && !field.enumOptions) {
    issue.addIssue({ code: 'custom', message: 'enumOptions are required for enum fields.' });
  }
  if (field.dataType !== 'enum' && field.enumOptions) {
    issue.addIssue({ code: 'custom', message: 'enumOptions require an enum field.' });
  }
  if (field.dataType === 'reference' && !field.referenceTableIds) {
    issue.addIssue({ code: 'custom', message: 'referenceTableIds are required for reference fields.' });
  }
  if (field.dataType !== 'reference' && field.referenceTableIds) {
    issue.addIssue({ code: 'custom', message: 'referenceTableIds require a reference field.' });
  }
});
...
export function registerWriteTools(server: McpServer, context: McpRequestContext): void {
  ...
  server.registerTool('create_table_row', {
    ...
    inputSchema: createRowSchema, annotations: writeAnnotations,
  }, async (input: z.infer<typeof createRowSchema>) => executeRpc(
    context, 'mcp_create_table_row', {
      p_project_id: context.projectId,
  ...
  const updateRowSchema = z.object({ tableId: uuid,
    ...
    values: z.record(z.string().trim().min(1).max(200), z.unknown()).refine(
      value => Object.keys(value).length > 0 && Object.keys(value).length <= 100,
      'values must contain between 1 and 100 fields.'),
  }).strict().refine(value => (value.rowId === undefined) !== (value.rowIndex === undefined),
    'Exactly one of rowId or rowIndex is required.');
  server.registerTool('update_table_row', {
    description: 'Update one row selected by stable ID or exact 1-based row index atomically.',
    inputSchema: updateRowSchema, annotations: writeAnnotations,
  }, async (input: z.infer<typeof updateRowSchema>) => executeRpc(
    context, 'mcp_update_table_row', {
      p_project_id: context.projectId,
      p_table_id: input.tableId,
  ...
  server.registerTool('create_document', {
    ...
    inputSchema: createDocumentSchema, annotations: writeAnnotations,
  }, async (input: z.infer<typeof createDocumentSchema>) => {
    try {
      assertUtf8Below(input.markdown, MAX_DOCUMENT_MARKDOWN_BYTES, 'Document Markdown');
      const data = await runMcpOperation(context, 'mcp_create_document', 'write', input, async () => {
        const normalized = await encodeDocumentMarkdown(input.markdown);
        return await rpc<unknown>(context, 'mcp_create_document', {
          p_project_id: context.projectId,
  ...
  const updateDocumentSchema = z.object({
    documentId: uuid,
    markdown: z.string(),
    stateToken: z.object({ epoch: z.number().int().min(0),
      revision: z.number().int().min(0), updateIds: z.array(uuid).max(10000) }).strict(),
  }).strict();
  server.registerTool('update_document', {
    description: 'Replace document Markdown with complete state-token conflict protection.',
    inputSchema: updateDocumentSchema, annotations: writeAnnotations,
  }, async (input: z.infer<typeof updateDocumentSchema>) => {
    try {
      assertUtf8Below(input.markdown, MAX_DOCUMENT_MARKDOWN_BYTES, 'Document Markdown');
      const data = await runMcpOperation(context, 'mcp_replace_document_content', 'write', input,
        async () => {
          const current = await readDocumentTransportState(context, input.documentId);
          const token = input.stateToken;
          const currentIds = current.tail.map(row => row.id);
          if (current.head.collab_epoch !== token.epoch ||
          ...
          const [merged, replacement] = await Promise.all([
            normalizeDocumentState(current.head.yjs_state,
              current.tail.map(row => row.update_data)),
            encodeDocumentMarkdown(input.markdown),
          ]);
          const url = Deno.env.get('SUPABASE_URL');
          const key = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY');
          if (!url || !key) throw new Error('Document replacement is unavailable.');
          const privileged = createClient(url, key, {
            auth: { persistSession: false, autoRefreshToken: false },
          });
          const result = await privileged.rpc('mcp_replace_document_content', {
            p_project_id: context.projectId,
          ...
          if (result.error) {
            if (result.error.code === 'PT409') throw new McpDomainError(
              'DOCUMENT_CONFLICT', 'Document changed; read it again before updating.');
            if (result.error.code === '42501') throw new McpDomainError(
              'WRITE_FORBIDDEN', 'The project is no longer writable.');
