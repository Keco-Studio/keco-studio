-- complete_image_upload returns metadata intended for update_table_row. Keep
-- image fields writable only when that metadata still matches the caller's
-- project-scoped Storage object.
