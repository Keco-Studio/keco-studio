-- Keep immutable version output, generation idempotency, and current metadata atomic.
...
create unique index if not exists game_design_system_versions_generation_job_idx
  on public.game_design_system_versions(generation_job_id)
