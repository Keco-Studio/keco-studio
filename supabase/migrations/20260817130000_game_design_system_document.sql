-- Store the human-readable design document beside the structured Agent rules.

alter table public.game_design_system_versions
  add column if not exists document jsonb;

-- Keep source snapshots protected by the column-level grants from the
-- snapshot-security migration while exposing the human-readable document.
grant select (document) on public.game_design_system_versions to authenticated;
...
begin
  ...
  insert into public.game_design_system_versions (
    system_id, version_number, parent_version_id, document, rules, rendered_markdown,
    source_snapshots, diff, conflicts, content_hash, created_by, generation_job_id
  ) values (
    p_system_id, v_version_number, p_parent_version_id, p_document, p_rules, v_rendered_markdown,
