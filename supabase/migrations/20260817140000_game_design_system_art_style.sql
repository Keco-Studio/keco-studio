-- Store an immutable, bounded visual specification beside each version.

alter table public.game_design_system_versions
  add column if not exists art_style jsonb;

alter table public.game_design_system_versions
  drop constraint if exists game_design_system_versions_art_style_object,
  drop constraint if exists game_design_system_versions_art_style_size;
...
grant select (art_style) on public.game_design_system_versions to authenticated;

drop function if exists public.create_game_design_system_version(
  uuid, uuid, jsonb, jsonb, text, jsonb, jsonb, jsonb, text, uuid, uuid
);
...
begin
  ...
  insert into public.game_design_system_versions (
    ...
    p_system_id, v_version_number, p_parent_version_id, p_document, p_art_style, p_rules, v_rendered_markdown,
...
revoke all on function public.create_game_design_system_version(
  uuid, uuid, jsonb, jsonb, jsonb, text, jsonb, jsonb, jsonb, text, uuid, uuid
