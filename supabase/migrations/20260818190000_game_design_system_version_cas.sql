-- Serialize version writes and make successful retries lookup-only.

alter table public.game_design_system_versions
  add column if not exists idempotency_key uuid;

create unique index if not exists game_design_system_versions_idempotency_idx
  on public.game_design_system_versions(system_id, idempotency_key)
...
begin
  ...
  if p_idempotency_key is not null then
    ...
    if found then
      if not (
        ...
        and v_version.content_hash = p_content_hash
  ...
  insert into public.game_design_system_versions (
    ...
    p_source_snapshots, p_diff, p_conflicts, p_content_hash, v_actor, p_generation_job_id, p_idempotency_key
