-- Follow up to 20260818200000: public keyed writes must replay or conflict
-- before a canonical no-op is rejected. Keyless internal writes retain their
-- previous behavior.
...
begin
  ...
  if p_idempotency_key is not null then
    ...
    if p_document is not distinct from v_parent_version.document
      and p_rules is not distinct from v_parent_version.rules
      and v_effective_art_style is not distinct from v_parent_version.art_style then
      raise exception 'VERSION_NO_CHANGES' using errcode = 'P0001';
    end if;
  end if;
