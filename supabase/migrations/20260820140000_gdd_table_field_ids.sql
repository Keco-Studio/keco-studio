-- Persist deterministic library_field_definitions.id values from worker fieldIds
-- so GDD ResourceReference chips remain stable across create/update.
...
begin
  ...
  for v_resource in select value from jsonb_array_elements(p_table_resources) loop
    ...
    if not v_existing_resource or v_series_resource.content_hash <> v_resource_hash then
      if jsonb_typeof(v_resource -> 'fieldIds') <> 'array'
        or jsonb_array_length(v_resource -> 'fieldIds') <> jsonb_array_length(v_resource -> 'fields') then
        raise exception 'Generated table fieldIds must match fields' using errcode = '22023';
      end if;
      v_row_index := 0;
      for v_field in select value from jsonb_array_elements(v_resource -> 'fields') loop
        ...
        begin
          v_field_id := (v_resource -> 'fieldIds' ->> v_row_index)::uuid;
        exception when others then
          raise exception 'Invalid generated table field ID' using errcode = '22023';
        end;
        if v_field_id is null then
          raise exception 'Invalid generated table field ID' using errcode = '22023';
        end if;
        insert into public.library_field_definitions(id, library_id, section, section_id, label, data_type, required, order_index)
        values (v_field_id, v_table_id, '__keco_flat_fields__', md5(v_table_id::text || '::keco-flat-fields'), v_field_label, 'string', false, v_row_index);
