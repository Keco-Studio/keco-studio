...
begin
  ...
  v_result := jsonb_build_object('ok', true, 'outcome', 'created', 'runId', p_run_id, 'stateToken', v_new_token, 'currentSequence', v_sequence, 'repairCount', v_run.repair_count, 'projection', v_projection);
...
begin
  ...
  if p_idempotency_key is null or length(p_idempotency_key) not between 8 and 128
    or p_idempotency_key !~ '^[A-Za-z0-9._:-]+$'
    or p_input_hash !~ '^sha256:[a-f0-9]{64}$'
    or p_expected_state_token is null then
  ...
  if coalesce(jsonb_typeof(p_documents), 'null') <> 'array'
    or jsonb_array_length(p_documents) <> (select count(*) from jsonb_object_keys(v_run.document_ids))
    or (select count(*) from jsonb_array_elements(p_documents)) <>
  ...
  for v_document in select value from jsonb_array_elements(p_documents) loop
    ...
    if not found
      ...
      or v_current.content is distinct from v_document->>'markdown'
  ...
  update public.keco_slice_runs set current_sequence = v_sequence, state_token = v_token, projection = v_projection, finalized_at = now(), updated_at = now() where id = p_run_id;
  v_result := jsonb_build_object('ok', true, 'outcome', 'created', 'runId', p_run_id, 'stateToken', v_token, 'currentSequence', v_sequence, 'projection', v_projection);
...
revoke all on function public.mcp_finalize_slice(uuid,uuid,uuid,jsonb,text,text) from public, anon;
...
grant execute on function public.mcp_finalize_slice(uuid,uuid,uuid,jsonb,text,text) to authenticated;
