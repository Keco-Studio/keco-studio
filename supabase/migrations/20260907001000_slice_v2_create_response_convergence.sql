-- Align the Slice V2 create response with the strict Edge response schema.
-- The preceding RPC convergence returned the retired legacyLayout field, so
-- successful transactions were reported to MCP clients as INTERNAL_ERROR.
...
update public.keco_slice_run_requests
set result = result - 'legacyLayout'
where operation = 'create_slice_bundle_v2'
  and result->>'contractVersion' = '2'
  and result ? 'legacyLayout';
