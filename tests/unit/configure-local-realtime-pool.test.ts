...
function createFixture(options: FixtureOptions = {}) {
  ...
  const sql: string[] = [];
  const waits: number[] = [];
  const terminationEffects: number[] = [];
  const terminationResults: string[] = [];
  ...
  const run: PoolConfiguratorDependencies['run'] = async (args) => {
    ...
    if (args[0] === 'exec') {
      ...
      if (terminatesStalePool) {
        livePool = options.livePoolAtTermination ?? livePool;
        const atomicallyRechecksState =
          statement.includes('activity_snapshot AS MATERIALIZED') &&
          statement.includes("pool_state.stored_pool = '10'") &&
          statement.includes('pool_state.live_pool > 0') &&
          statement.includes('pool_state.live_pool <> 10');
        const staleAtTermination =
          storedPool === 10 && livePool > 0 && livePool !== 10;
        const terminates = atomicallyRechecksState ? staleAtTermination : livePool > 0;

        if (terminates) {
          terminationEffects.push(livePool);
          if (options.terminationBecomesReady ?? true) {
            livePool = 0;
          }
        }

        const result = terminates ? 'terminated' : 'skipped';
...
describe('configureLocalRealtimePool', () => {
  ...
  it('skips termination when a stale initial count has already recovered to ten', async () => {
    const fixture = createFixture({
      storedPool: 10,
      livePool: 3,
      livePoolAtTermination: 10,
    });
    ...
    expect(fixture.terminationResults).toEqual(['skipped']);
    expect(fixture.terminationEffects).toEqual([]);
    const terminateSql = fixture.sql.find((statement) =>
      statement.includes('pg_terminate_backend')
    );
    expect(terminateSql).toContain('activity_snapshot AS MATERIALIZED');
    expect(terminateSql?.match(/FROM activity_snapshot/g)).toHaveLength(2);
  });
  ...
  it.each([
    [
      'LOCAL_REALTIME_GUARD_TENANT',
      'Expected exactly one local Realtime tenant',
    ],
    [
    ...
    [
      'LOCAL_REALTIME_GUARD_DB_POOL_NUMERIC',
      'Local Realtime db_pool must be numeric',
    ],
  ])('maps %s to a clear error without exposing command details', async (code, message) => {
    const fixture = createFixture({
      sqlError: new Error(
        `Command failed: docker exec psql -c SELECT settings; ERROR: ${code}; ` +
          'DETAIL: {"db_password":"do-not-print"}'
      ),
    });
    ...
    expect((received as Error).message).toBe(message);
    expect((received as Error).message).not.toContain('Command failed');
    expect((received as Error).message).not.toContain('SELECT');
    expect((received as Error).message).not.toContain('do-not-print');
  });

  it('does not map a guard code that appears only inside echoed SQL', async () => {
    const fixture = createFixture({
      sqlError: new Error(
        "Command failed: psql -c RAISE EXCEPTION 'LOCAL_REALTIME_GUARD_TENANT'; " +
    ...
    await expect(
      configureLocalRealtimePool({ run: fixture.run, wait: fixture.wait })
    ).rejects.toThrow('Failed to query local Realtime pool');
  });
});
