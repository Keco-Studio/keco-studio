...
describe('GDD resource version evolution migration', () => {
  it('creates one stable resource series for each project and design system', () => {
    ...
    expect(sql).toMatch(/folder_id uuid references public\.folders\(id\)/i);
    expect(sql).toMatch(/primary_document_id uuid references public\.documents\(id\)/i);
  ...
  it('tracks stable logical resources with exclusive document or library ownership', () => {
    ...
    expect(sql).toMatch(/logical_key text not null check \(logical_key = lower\(btrim\(logical_key\)\) and char_length\(logical_key\) between 1 and 160\)/i);
    expect(sql).toMatch(/document_id uuid references public\.documents\(id\) on delete set null/i);
    expect(sql).toMatch(/library_id uuid references public\.libraries\(id\) on delete set null/i);
  ...
  it('links generation jobs to a revisioned series and validates change summaries', () => {
    ...
    expect(sql).toMatch(/alter table public\.gdd_generation_jobs[\s\S]*add column if not exists generation_series_id uuid references public\.gdd_resource_series\(id\) on delete set null/i);
