...
describe('desktop release workflow', () => {
  ...
  it('builds each macOS release for its portable CPU baseline', () => {
    ...
    expect(workflow).toContain('zig build -Dtarget=x86_64-macos -Dplatform=macos -Doptimize=ReleaseFast');
    expect(workflow).toContain('zig build -Dtarget=aarch64-macos -Dplatform=macos -Doptimize=ReleaseFast');
  });
