...
describe("Keco Godot Slice V2 skill contract", () => {
  it("requires checklist-based authoritative Keco roadmap and Slice plans", () => {
    ...
    expect(orchestration).toMatch(
      /Keco(?:'s|’s) read-back plan is authoritative/i,
    );
  ...
  it("keeps V2 as the canonical workflow for document-driven Godot creation", () => {
    ...
    expect(skill).toMatch(
      /V2 is the canonical creation workflow for Keco-driven Godot development/i,
    );
  ...
  it("supports document-driven implicit invocation and exposes a self-contained reviewed ledger", () => {
    ...
    expect(skill).not.toContain(
      "explicitly selects `$keco-develop-godot-slice-v2`",
    );
    expect(skill).toMatch(
      /INTAKE[\s\S]*WRITE_PLAN[\s\S]*TASK_REVIEW[\s\S]*FINAL_VERIFY/,
    );
    ...
    expect(skill).toMatch(
      /already consistent[\s\S]*without asking|without asking[\s\S]*already consistent/i,
    );
    expect(skill).toMatch(
      /unresolved ambiguity[\s\S]*zero writes|zero writes[\s\S]*unresolved ambiguity/i,
    );
    expect(skill).toMatch(/multiple independent[\s\S]*not[\s\S]*ambiguity/i);
    expect(skill).toMatch(
      /planning-document preflight[\s\S]*execution preflight/i,
    );
    expect(skill).toMatch(
      /spec\.md[\s\S]*plan\.md[\s\S]*status\.json[\s\S]*eval-report\.json/i,
    );
    expect(skill).toMatch(
      /matching Keco Project[\s\S]*create_document\(projectId, folderId/i,
    );
    expect(skill).toMatch(
      /local mirror[\s\S]*never as the only copy|never as the only copy[\s\S]*local mirror/i,
    );
  ...
  it("ships all contracts and deterministic validators", () => {
    ...
    const assets = readFileSync(
      path.join(skillRoot, "references", "keco-pixellab-contract.md"),
      "utf8",
    );
    expect(assets).toMatch(
      /planned row[\s\S]*read-back[\s\S]*create_image_upload[\s\S]*complete_image_upload/i,
    );
    expect(assets).toMatch(/API key[\s\S]*(?:environment|MCP configuration)/i);
    expect(assets).toMatch(/Generated Assets/);
    expect(assets).toMatch(
      /assetKind[\s\S]*providerCapability[\s\S]*transportTool[\s\S]*compatibility/i,
    );
    expect(assets).not.toMatch(/create_s_xl_image_pro` is a legacy/i);
    const capabilityRegistry = readFileSync(
      path.join(skillRoot, "references", "pixellab-capability-registry.md"),
      "utf8",
    );
    expect(capabilityRegistry).toMatch(
      /generate_image_pixflux[\s\S]*generate_image_bitforge/i,
    );
    ...
    const godot = readFileSync(
      path.join(skillRoot, "references", "godot-mcp-contract.md"),
      "utf8",
    );
    ...
    expect(sourceData).toMatch(
      /semantic field labels[\s\S]*stable scalar match keys/i,
    );
    expect(sourceData).toMatch(
      /semantic[\s\S]*clearly dominant[\s\S]*awaiting_user_confirmation/i,
    );
    expect(sourceData).toMatch(/arbitrary[\s\S]*source document names/i);
    expect(sourceData).toMatch(
      /(?:no|not)[\s\S]*(?:fixed[\s\S]*Feedback|Feedback[\s\S]*fixed)/i,
    );
    expect(sourceData).toMatch(
      /(?:no|not)[\s\S]*(?:recency|latest)[\s\S]*(?:alone|only)/i,
    );
    expect(sourceData).toMatch(
      /exactly one[\s\S]*clearly dominant[\s\S]*(?:auto-select|automatically select)/i,
    );
    expect(sourceData).toMatch(
      /tied candidates[\s\S]*one focused question[\s\S]*zero writes/i,
    );
    expect(sourceData).toMatch(/never automatically delete/i);
    expect(sourceData).toMatch(
      /canonical Keco Project[\s\S]*kecoFolderId[\s\S]*pre-write blocker/i,
    );
    const evalContract = readFileSync(
      path.join(skillRoot, "references", "eval-contract.md"),
      "utf8",
    );
    expect(evalContract).toMatch(
      /Create EvalSpec before any Keco, PixelLab, or Godot write/i,
    );
    const reviewWorkflow = readFileSync(
      path.join(skillRoot, "references", "review-workflow.md"),
      "utf8",
    );
    expect(reviewWorkflow).toMatch(
      /Plan validation[\s\S]*Task RED\/GREEN[\s\S]*Independent completion review/i,
    );
    expect(reviewWorkflow).not.toMatch(/superpowers/i);
    const generatedAssets = readFileSync(
      path.join(skillRoot, "references", "generated-asset-contract.md"),
      "utf8",
    );
    ...
    const evolution = readFileSync(
      path.join(skillRoot, "references", "existing-resource-evolution.md"),
      "utf8",
    );
    ...
    const animation = readFileSync(
      path.join(skillRoot, "references", "godot-animation-contract.md"),
      "utf8",
    );
    expect(animation).toMatch(
      /SpriteFrames[\s\S]*AtlasTexture[\s\S]*AnimatedSprite2D/i,
    );
    expect(animation).toMatch(
      /frameCount[\s\S]*frameWidth[\s\S]*fps[\s\S]*loop/i,
    );
    expect(animation).toMatch(/packaged export[\s\S]*materialize/i);
    const tileset = readFileSync(
      path.join(skillRoot, "references", "godot-tileset-contract.md"),
      "utf8",
    );
    expect(tileset).toMatch(/TileSet[\s\S]*TileMapLayer[\s\S]*terrain/i);
    expect(tileset).toMatch(
      /topdown-15[\s\S]*platformer-16[\s\S]*isometric-atlas/i,
    );
    ...
    expect(sliceDocuments).toMatch(
      /docs\/keco-godot-slices[\s\S]*spec\.md[\s\S]*plan\.md[\s\S]*status\.json/i,
    );
    expect(sliceDocuments).toMatch(/latest[\s\S]*superseded[\s\S]*completed/i);
    expect(sliceDocuments).toMatch(
      /Keco documents are authoritative[\s\S]*create_document\(projectId, folderId[\s\S]*read_document/i,
    );
    expect(sliceDocuments).toMatch(
      /no compatible folder[\s\S]*blocked_before_write/i,
    );
    const sliceDecision = readFileSync(
      path.join(skillRoot, "references", "slice-decision.md"),
      "utf8",
    );
    expect(sliceDecision).toMatch(
      /consistent[\s\S]*without[\s\S]*confirmation/i,
    );
    expect(sliceDecision).toMatch(
      /awaiting_user_confirmation[\s\S]*zero-write/i,
    );
    const orchestration = readFileSync(
      path.join(skillRoot, "references", "orchestration-contract.md"),
      "utf8",
    );
    expect(orchestration).toMatch(/specPath[\s\S]*planPath[\s\S]*statusPath/i);
    expect(orchestration).toMatch(
      /kecoFolderId[\s\S]*kecoDocumentIds[\s\S]*localMirrorRoot/i,
    );
    expect(orchestration).toMatch(
      /evolution[\s\S]*reuse_exact[\s\S]*create_new/i,
    );
    expect(orchestration).toMatch(
  ...
  it("ships the multi-Slice roadmap and recovery contract", () => {
    const multiSlicePath = path.join(
      skillRoot,
      "references",
      "multi-slice-orchestration.md",
    );
    ...
    expect(multiSlice).toMatch(
      /all planned[\s\S]*Slices[\s\S]*sequentially[\s\S]*dependencies[\s\S]*complete/i,
    );
    expect(multiSlice).toMatch(/priority[\s\S]*tie-breaker/i);
    expect(multiSlice).toMatch(
      /third failed repair iteration[\s\S]*persist[\s\S]*evidence[\s\S]*status[\s\S]*eval-report/i,
    );
    expect(multiSlice).toMatch(
      /third failed repair iteration[\s\S]*roadmap[\s\S]*paused[\s\S]*NEXT_SLICE[\s\S]*clear[\s\S]*ask[\s\S]*user/i,
    );
    expect(multiSlice).toMatch(
      /status: planned\|in_progress\|completed\|failed\|blocked[\s\S]*evalResult: passed\|partial\|failed\|blocked_before_write/i,
    );
  });
  ...
  it("builds deterministic Godot SpriteFrames resources and rejects bad frame geometry", () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          ...
          animations: [
            {
              name: "idle",
              sheetPath: "res://generated/hero_idle.png",
              sheetFile: idle,
              frameWidth: 32,
              frameHeight: 32,
              frameCount: 4,
            ...
            {
              name: "attack",
              sheetPath: "res://generated/hero_attack.png",
              sheetFile: attack,
              frameWidth: 32,
              frameHeight: 32,
              frameCount: 3,
      ...
      const valid = spawnSync(
        "python3",
        [builder, "--manifest", manifest, "--output", output],
        { encoding: "utf8" },
      );
  ...
  it("validates generated asset packages before Godot materialization", () => {
    ...
    try {
        ...
        assets: [
          {
            ...
            provider: {
              ...
              transportTool: "animate_with_text",
              assetId: "provider-idle",
            },
            status: "ready",
            files: [
              {
                fileKey: "hero-idle-sheet",
                sourceFile: "assets/hero_idle.png",
                targetPath: "res://generated/hero_idle.png",
                sha256: sha256(idle),
                width: 128,
                height: 32,
                animation: {
          ...
          },
          {
            ...
            assetKind: "tileset",
            provider: {
              capability: "top-down-tileset",
              transportTool: "generate_tileset",
              assetId: "provider-terrain",
            },
            status: "ready",
            files: [
              {
                fileKey: "terrain-grass-atlas",
                sourceFile: "assets/terrain.png",
                targetPath: "res://generated/terrain.png",
                ...
                tileset: {
                  ...
                  columns: 4,
                  rows: 4,
                  terrainMapping: "provider-metadata",
                },
              },
            ],
          },
      ...
      const valid = spawnSync("python3", [validator, packagePath], {
        encoding: "utf8",
      });
      expect(valid.status).toBe(0);
      expect(valid.stdout).toMatch(/"ok": true/);

      packageData.assets[1].files[0].targetPath =
        "res://generated/hero_idle.png";
      writeFileSync(packagePath, JSON.stringify(packageData));
      const invalid = spawnSync("python3", [validator, packagePath], {
        encoding: "utf8",
      });
      expect(invalid.status).not.toBe(0);
  ...
  it("validates dated slice documents and completion state", () => {
    ...
    try {
      ...
      writeFileSync(
        path.join(sliceDir, "status.json"),
        JSON.stringify({
          version: 1,
          sliceId: "hero-animation",
          createdDate: "2026-08-06",
      ...
      const valid = spawnSync("python3", [validator, "--slice-dir", sliceDir], {
        encoding: "utf8",
      });
      expect(valid.status).toBe(0);
      expect(valid.stdout).toMatch(/"ok": true/);
    ...
    const fixture = JSON.parse(
      readFileSync(
        path.join(
          ...
          "fixtures",
          "plugins",
          "keco-godot-skill-v2-evals.json",
        ),
        "utf8",
      ),
    ) as { cases: unknown[]; invocation: string };
    ...
    try {
      ...
      expect(() =>
        execFileSync("python3", [
          path.join(skillRoot, "scripts", "validate_run_context.py"),
          unsafeRun,
        ]),
      ).toThrow();
      ...
      writeFileSync(
        ...
        JSON.stringify({
          tasks: [
            { id: "task-01", files: [], dependsOn: [], servesEvaluations: [] },
          ],
        }),
      );
      expect(() =>
        execFileSync("python3", [
          path.join(skillRoot, "scripts", "validate_plan.py"),
          incompletePlan,
        ]),
      ).toThrow();
    } finally {
  ...
  it("rejects runtime and evidence state in a reviewed plan", () => {
    const tempRoot = mkdtempSync(
      path.join(os.tmpdir(), "keco-v2-plan-boundary-"),
    );
    try {
  ...
  it("keeps snapshot export and validation runnable inside v2", () => {
    ...
    try {
      const exporter = spawnSync(
        "python3",
        [
          path.join(skillRoot, "scripts", "export_keco_snapshot.py"),
          "--input",
          input,
          "--output-dir",
          output,
      ...
      const validator = spawnSync(
        "python3",
        [
          path.join(skillRoot, "scripts", "validate_snapshot.py"),
          "--snapshot-dir",
          output,
          "--source-input",
          input,
