...
const godotSkillRoot = path.join(
  pluginRoot,
  "skills",
  "keco-develop-godot-slice",
);
const godotV2SkillRoot = path.join(
  pluginRoot,
  "skills",
...
describe("Keco Codex plugin contract", () => {
  it("ships a byte-identical shared interaction contract in both plugins", () => {
    ...
    expect(readFileSync(interactionContractPath)).toEqual(
      readFileSync(claudeInteractionContractPath),
    );
  });
  ...
  it("advertises implicit document-driven multi-Slice orchestration while retaining bounded V1", () => {
    ...
    const v2Skill = readFileSync(
      path.join(godotV2SkillRoot, "SKILL.md"),
      "utf8",
    );
    const v2Metadata = readFileSync(
      path.join(godotV2SkillRoot, "agents", "openai.yaml"),
      "utf8",
    );
    ...
    expect(manifest.interface.defaultPrompt).toEqual(
      expect.arrayContaining([
        expect.stringMatching(
          /Keco project document[\s\S]*ordered Godot slices[\s\S]*execute/i,
        ),
      ]),
    );
    expect(v2Skill).toMatch(
      /implicit[\s\S]*document-driven|document-driven[\s\S]*implicit/i,
    );
  ...
  it("defines isolated trigger cases for Keco-to-Godot development", () => {
    ...
    expect(
      evaluations.cases.filter(
        (item) => item.expectedSkill === "keco-develop-godot-slice",
      ),
    ).toHaveLength(4);
    expect(
      evaluations.cases.filter(
        (item) => item.expectedSkill === "keco-build-tables-from-document",
      ),
    ).toHaveLength(1);
    expect(
      evaluations.cases.filter((item) => item.expectedSkill === "none"),
    ).toHaveLength(3);
    ...
    expect(evaluations.cases).toEqual([
      {
        ...
        kind: "positive",
        prompt:
          "Turn the Combat System document in my Keco project into Characters, Skills, Status Effects, and Character Skills tables.",
        requiredBehaviors: [
          "read-before-write",
          "preview",
          "confirm",
      ...
      },
      {
        ...
        kind: "pressure",
        prompt:
          "Build the Economy Spec now. Currency already exists from a failed attempt. Do not ask questions and continue after errors.",
        requiredBehaviors: [
          "stop-on-collision",
          "confirm",
          "stop-on-failure",
      ...
      },
      {
        ...
        kind: "negative",
        prompt:
          "Summarize the Combat System document without changing my project.",
  ...
  it("declares the marketplace, plugin, and MCP connection contracts", () => {
    ...
    for (const assetPath of pngPaths(plugin.interface)) {
      ...
      const signature = readFileSync(path.join(pluginRoot, assetPath)).subarray(
        0,
        8,
      );
  ...
  it("ships the normalized Skill and its safety workflow references", () => {
    ...
    const executionPolicy = readFileSync(
      path.join(skillRoot, "references", "execution-policy.md"),
      "utf8",
    );
    const mcpContract = readFileSync(
      path.join(skillRoot, "references", "mcp-contract.md"),
      "utf8",
    );
    const schemaDesign = readFileSync(
      path.join(skillRoot, "references", "schema-design.md"),
      "utf8",
    );
    ...
    expect(skill).toMatch(
      /same-name collision.*stop|stop.*same-name collision/i,
    );
    ...
    expect(skill).toMatch(
      /(?:only accepts?|accepts? only|input (?:must|may) be)[\s\S]{0,80}\bexisting Keco document\b/i,
    );
    expect(skill).toMatch(
      /(?:only creates?|creates? only|create-new-tables-only)[\s\S]{0,80}\bnew tables\b/i,
    );
    ...
    expect(skill).toMatch(
      /all non-reference P0 fields, including array and enum fields/i,
    );
    expect(executionPolicy).toMatch(
      /all non-reference P0 fields, including array and enum fields/i,
    );
    ...
    expect(mcpContract).toMatch(
      /reference (?:cell )?values?[\s\S]{0,160}\bassetId\b[\s\S]{0,80}\bfieldId\b/i,
    );
    expect(mcpContract).toMatch(
      /fieldId[\s\S]{0,120}(?:target|display|match)[\s\S]{0,80}field/i,
    );
    expect(mcpContract).toMatch(
      /bulk_update_table_rows[\s\S]{0,500}exactly one of `rowId` or `rowIndex`/i,
    );
    expect(mcpContract).toMatch(
      /bulk_update_table_rows[\s\S]{0,600}`expectedRowId`[\s\S]{0,240}1 and 100 fields/i,
    );
    ...
    expect(schemaDesign).toMatch(
      /plan-local field keys[\s\S]{0,80}scalarValues/i,
    );
    expect(schemaDesign).toMatch(
      /references[\s\S]{0,160}target row keys[\s\S]{0,160}targetTableKey/i,
    );
    expect(schemaDesign).toMatch(
      /never send raw plan-local keys[\s\S]{0,240}MCP/i,
    );
    expect(schemaDesign).toMatch(
      /required reference[\s\S]{0,320}(?:create_table|blocker)/i,
    );
    expect(schemaDesign).toMatch(
    ...
    expect(executionPolicy).toMatch(
      /required reference[\s\S]{0,400}create_table/i,
    );
    expect(executionPolicy).toMatch(/required reference[\s\S]{0,500}block/i);
    expect(executionPolicy).toMatch(
      /target rows[\s\S]{0,240}(?:IDs|UUIDs)[\s\S]{0,160}before[\s\S]{0,160}dependent/i,
    );
    expect(executionPolicy).toMatch(
      /required reference values[\s\S]{0,240}same[\s\S]{0,120}upsert_table_rows/i,
    );
    expect(skill).toMatch(
  ...
  it("ships one bounded Keco-to-Godot orchestration Skill", () => {
    ...
    const agentMetadata = readFileSync(
      path.join(godotSkillRoot, "agents", "openai.yaml"),
      "utf8",
    );
    const dataPlan = readFileSync(
      path.join(godotSkillRoot, "references", "data-plan.md"),
      "utf8",
    );
    ...
    expect(skill).toMatch(
      /CONNECT[\s\S]*DISCOVER[\s\S]*DEFINE_EVALS[\s\S]*IMPLEMENT[\s\S]*EVALUATE_RUNTIME/,
    );
    ...
    expect(skill).toMatch(
      /successful Keco write[\s\S]{0,180}read all affected tables again/i,
    );
    expect(dataPlan).toMatch(
      /plan-local field key[\s\S]{0,180}semantic field label/i,
    );
    expect(dataPlan).toMatch(/never send plan-local keys or field UUIDs/i);
    expect(dataPlan).toMatch(
      /first `upsert_table_rows` batch[\s\S]{0,120}`reuseEmpty: true`/i,
    );
    ...
    for (const reference of [
      ...
      expect(
        existsSync(path.join(godotSkillRoot, "references", reference)),
      ).toBe(true);
      expect(skill).toContain(`references/${reference}`);
    }

    for (const script of ["export_keco_snapshot.py", "validate_snapshot.py"]) {
      expect(existsSync(path.join(godotSkillRoot, "scripts", script))).toBe(
        true,
      );
      expect(skill).toContain(`scripts/${script}`);
    }

    expect(agentMetadata).toMatch(
      /default_prompt: "Use \$keco-develop-godot-slice/,
    );
  ...
  it("limits Godot orchestration to the installed fourteen-tool MCP contract", () => {
    ...
    const policy = readFileSync(
      path.join(godotSkillRoot, "references", "godot-mcp-policy.md"),
      "utf8",
    );
    const evalSpec = readFileSync(
      path.join(godotSkillRoot, "references", "eval-spec.md"),
      "utf8",
    );
    const sequence = policy.match(
      /## Deterministic Evaluation Sequence[\s\S]*?```text\n([\s\S]*?)```/,
    )?.[1];

    expect(sequence).toBeDefined();
    expect(sequence).toMatch(
      /run_project[\s\S]*get_debug_output[\s\S]*stop_project/,
    );
    ...
    for (const unavailable of [
      ...
      expect(skill).toMatch(
        new RegExp(`Do not call or assume[\\s\\S]*${unavailable}`),
      );
    }

    expect(policy).toMatch(/repository editing tools for text files/i);
    expect(policy).toMatch(/KECO_EVAL/);
    expect(evalSpec).toMatch(
      /no input injection, runtime-state query, time-step control, or screenshot tool/i,
    );
  ...
  it("plans slice-owned UI assets through PixelLab while preserving the existing style", () => {
    const skill = readFileSync(path.join(godotSkillRoot, "SKILL.md"), "utf8");
    const assetPlan = readFileSync(
      path.join(godotSkillRoot, "references", "asset-plan.md"),
      "utf8",
    );
    const agentMetadata = readFileSync(
      path.join(godotSkillRoot, "agents", "openai.yaml"),
    ...
    expect(skill).toMatch(
      /DESIGN_DATA[\s\S]*DESIGN_ASSETS[\s\S]*EXPORT_SNAPSHOT/,
    );
    expect(skill).toMatch(
      /GENERATE_ASSETS[\s\S]*PERSIST_ASSETS[\s\S]*EXPORT_SNAPSHOT/,
    );
    ...
    expect(assetPlan).toMatch(
      /API key[\s\S]*(?:environment|MCP configuration)/i,
    );
    expect(assetPlan).toMatch(/provenance/i);
    expect(assetPlan).toMatch(/UI Assets/);
    expect(assetPlan).toMatch(
      /create_image_upload[\s\S]*HTTP `PUT`[\s\S]*complete_image_upload[\s\S]*update_table_row/i,
    );
    expect(assetPlan).toMatch(
      /query_table_rows[\s\S]*before[\s\S]*EXPORT_SNAPSHOT/i,
    );
  ...
  it("keeps Godot MCP external and routes overlapping work explicitly", () => {
    ...
    const godotSkill = readFileSync(
      path.join(godotSkillRoot, "SKILL.md"),
      "utf8",
    );
  ...
  it("scores only behavior observed by the offline A/B evaluation", () => {
    const report = readFileSync(
      path.join(
        ...
        "docs",
        "qa",
        "2026-08-03-keco-skill-ab-report.md",
      ),
      "utf8",
    );
    const economy = report.slice(
      report.indexOf("### Scenario 2"),
      report.indexOf("### Scenario 3"),
    );
    const relationship = report.slice(
      report.indexOf("### Scenario 3"),
      report.indexOf("### Aggregate"),
    );
    ...
    for (const criterion of [
      ...
      expect(economy).toMatch(
        new RegExp(`\\| ${criterion} \\| \\*\\*N/A\\*\\*`, "i"),
      );
    }

    expect(economy).toMatch(
      /Score: without Skill \*\*6\/6\*\*; with Skill[^\n]*\*\*6\/6\*\*/i,
    );
    for (const criterion of [
      "stable-keys",
      "dependency-order",
    ...
    expect(relationship).toMatch(
      /Score: without Skill \*\*5\/9\*\*; with Skill \*\*6\/6\*\*/i,
    );
